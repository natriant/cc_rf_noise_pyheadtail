...
stdPhaseNoise = 1e-8#*np.sqrt(0.72)  # Size of phase noise (1e-8 for ~24nm/s at 0 phaseGain), np.sqrt(0.72) for CC at 200 MHz
stdDipoleNoise = 1e-8/np.sqrt(2)

damperOn = 1  # Turns on the damper - 0 is off, 1 is on
dampingrate_x = 0 #50  # Strength of the damper (note it must be turned on further down in the code)
dampingrate_y= 40 # units? is this alpha in the paper??                           #(40 is the "standard" value)
