...
for i in range(n_turns):
    ...
    #bunch.yp += phaseKicks[i]*np.cos(2*np.pi*400e6/(bunch.beta*c)*bunch.z+150*deg2rad)
    if i == 0:
        bunch.yp += [cc_voltage(i)] * np.sin(2 * np.pi * 400e6 / (bunch.beta * c) * bunch.z+30*deg2rad)/(gamma * .938e9) 
    else:
        bunch.yp += [cc_voltage(i)] * np.sin(2 * np.pi * 400e6 / (bunch.beta * c) * bunch.z+phaseKicks[i]*P0C/cc_voltage(i)+30*deg2rad)/(gamma * .938e9)  # phase and amplitude noise
