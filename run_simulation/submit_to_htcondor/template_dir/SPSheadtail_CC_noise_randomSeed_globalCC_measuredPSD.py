...
charge = e
mass = m_p
intensity = 3.0e10 #2.95e10 

epsn_x = 2.3e-6 #2.08e-6 # m
epsn_y = 2.3e-6 #4.37e-6 # m
...
n_set = %n_set
PN_kicks = pickle.load(open(f'/afs/cern.ch/work/n/natriant/private/pyheadtail_example_crabcavity/timeSeriesPickleFiles_from_measuredNoise/time_series_from_Coast1-Setting2-PN_set{n_set}.pkl', 'rb'))
AN_kicks = pickle.load(open(f'/afs/cern.ch/work/n/natriant/private/pyheadtail_example_crabcavity/timeSeriesPickleFiles_from_measuredNoise/time_series_from_Coast1-Setting2-AN_set{n_set}.pkl', 'rb'))
...
for i in range(n_turns):
    ...
    # AN only
    bunch.yp += [cc_voltage(i) + cc_voltage(i) * AN_kicks[i]] * np.sin(2 * np.pi * 400e6 / (bunch.beta * c) * bunch.z)/(gamma * .938e9)  # amplitude noise
    # PN only
    #bunch.yp += [cc_voltage(i)] * np.sin(2 * np.pi * 400e6 / (bunch.beta * c) * bunch.z+PN_kicks[i])/(gamma * .938e9)  # phase and amplitude noise
