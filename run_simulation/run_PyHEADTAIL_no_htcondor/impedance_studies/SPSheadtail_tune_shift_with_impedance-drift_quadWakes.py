...
# only dipolar kick
my_length = len(ww1.wake_table['quadrupole_y'])
...
plt.plot(ww1.wake_table['time'], ww1.wake_table['quadrupole_y'])
plt.show()
plt.close()
...
plt.scatter(Z, (YP-bunch.yp)/bunch.y)
#plt.scatter(Z, XP-bunch.xp)

plt.ylabel(r'$\Delta py/y \ [rad]$')
#plt.tight_layout()
...
#plt.savefig('quad_kick.png', bbox_inches='tight')
...
wake_strength =  -convert_to_V_per_Cm*ww1.wake_table['quadrupole_y'] # dz = beta*c*dt, dz<0 for ultrarelativistic 
...
p_ix = my_sliceSet.particles_within_cuts_slice
print(p_ix)
print(bunch.y[p_ix])
...
#quit()
conv_dipole_y = np.convolve(my_sliceSet.n_macroparticles_per_slice, wake_interpolated[::-1], 'same')#*bunch.y[p_ix]
#len(conv_dipole_y)
...
plt.scatter(Z, (YP-bunch.yp)/bunch.y, label='simulation')

plt.plot(z_centers, 3*wake_kicks*1e4, c='r', label='theory')

plt.ylabel(r'$\Delta yp/y \ [rad/m]$')
plt.xlabel('z [m]')
plt.grid()
plt.legend()
plt.tight_layout()
